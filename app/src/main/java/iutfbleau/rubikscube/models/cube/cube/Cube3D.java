...
public class Cube3D extends Cube
{
    ...
    public Cube3D(int size)
    {
        ...
        ((Face3D)this.faces[0]).setPosition(-CUBE_SIZE,CUBE_SIZE,CUBE_SIZE,CUBE_SIZE,0.05f);
        ((Face3D)this.faces[0]).rotateX(Math.toRadians(-90.0));

        this.faces[1] = new Face3D(size, Piece.GREEN);
        ((Face3D)this.faces[1]).setPosition(-CUBE_SIZE,CUBE_SIZE,CUBE_SIZE,CUBE_SIZE,0.05f);
        ((Face3D)this.faces[1]).rotateY(Math.toRadians(-90.0));

        this.faces[2] = new Face3D(size, Piece.RED);
        ((Face3D)this.faces[2]).setPosition(-CUBE_SIZE,CUBE_SIZE,CUBE_SIZE,CUBE_SIZE,0.05f);

        this.faces[3] = new Face3D(size, Piece.BLUE);
        ((Face3D)this.faces[3]).setPosition(-CUBE_SIZE,CUBE_SIZE,CUBE_SIZE,CUBE_SIZE,0.05f);
        ((Face3D)this.faces[3]).rotateY(Math.toRadians(90.0));

        this.faces[4] = new Face3D(size, Piece.ORANGE);
        ((Face3D)this.faces[4]).setPosition(-CUBE_SIZE,CUBE_SIZE,CUBE_SIZE,CUBE_SIZE,0.05f);
        ((Face3D)this.faces[4]).rotateY(Math.toRadians(180.0));

        this.faces[5] = new Face3D(size, Piece.YELLOW);
        ((Face3D)this.faces[5]).setPosition(-CUBE_SIZE,CUBE_SIZE,CUBE_SIZE,CUBE_SIZE,0.05f);
